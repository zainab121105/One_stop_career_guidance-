...
const milestoneSchema = new mongoose.Schema({
  ...
  category: {
    type: String,
    required: true,
    enum: ['skill', 'education', 'experience', 'certification', 'project', 'networking']
  },
  estimatedDuration: {
    type: String,
  ...
  resources: [{
    type: {
      type: String,
      enum: ['course', 'book', 'website', 'certification', 'tool', 'practice']
    },
    title: String,
    url: String,
    description: String,
...
const careerRoadmapSchema = new mongoose.Schema({
  ...
  personalizedRecommendations: [{
    ...
    category: {
      type: String,
      enum: ['learning', 'networking', 'projects', 'certifications', 'experience']
    },
    priority: {
      type: String,
      enum: ['high', 'medium', 'low']
...
careerRoadmapSchema.virtual('completionPercentage').get(function() {
  ...
  return Math.round((this.overallProgress.completedMilestones / this.overallProgress.totalMilestones) * 100);
});
...
careerRoadmapSchema.methods.updateProgress = function() {
  ...
  this.phases.forEach(phase => {
    ...
    completedMilestones += phase.milestones.filter(milestone => milestone.completed).length;
  });
  
  this.overallProgress.totalMilestones = totalMilestones;
  this.overallProgress.completedMilestones = completedMilestones;
  this.overallProgress.percentageComplete = totalMilestones > 0 
...
careerRoadmapSchema.methods.completeMilestone = function(phaseId, milestoneId, notes = '') {
...
careerRoadmapSchema.methods.generateCacheKey = function() {
  ...
  const key = `roadmap_${profile.currentLevel}_${profile.careerStage}_${profile.interests.sort().join('_')}_${profile.goals.sort().join('_')}_${profile.timeCommitment}`;
