  ...
  async connectDB() {
    try {
      await mongoose.connect(
        process.env.MONGODB_URI || "mongodb://localhost:27017/careerpath_test"
      );
  ...
  async testRoadmapGeneration() {
    ...
    try {
      ...
      if (
        ...
        process.env.GOOGLE_AI_API_KEY === "your-google-ai-api-key-for-gemini"
      ) {
        console.log(
          "⚠️  Google AI API key not configured - testing with mock data"
        );
      ...
      console.log(
        `📊 Total milestones: ${roadmap.overallProgress.totalMilestones}`
      );
      ...
      console.log(
        `📊 Cache performance improvement: ${Math.round(
          ((generationTime - cacheTime) / generationTime) * 100
        )}%`
      );
      ...
      if (
        ...
        roadmap.phases[0].milestones.length > 0
      ) {
        const firstPhase = roadmap.phases[0];
        const firstMilestone = firstPhase.milestones[0];

        const updatedRoadmap =
          await this.roadmapService.updateMilestoneProgress(
            roadmap._id,
            firstPhase.id,
            firstMilestone.id,
            true,
        ...
        console.log(
          `📊 Updated progress: ${updatedRoadmap.overallProgress.percentageComplete}%`
        );
      }
      ...
      const regeneratedRoadmap = await this.roadmapService.regenerateRoadmap(
        user._id,
        "Test regeneration"
      );
